...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error("Erro ao criar cobrança com boleto:", error.message, error.stack);
      res.status(500).json({
        message: "Erro interno do servidor ao criar cobrança com boleto.",
        error: error.message, // Inclui a mensagem de erro na resposta (opcional)
      });
    }
  }
...
console.log('customer depois', data.customer)
        ...
        const pix = new PixQRcode({
          billingType: "PIX",
          custumerId: custumerId,
          customer: customer, // MongoDB ObjectId reference to the customer document
          value: data.value, // Ajuste para usar o valor correto
